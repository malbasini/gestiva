...
@Service
public class VatPurchaseRegisterXlsxExportService {

    public byte[] export(List<VatPurchaseRegisterRowView> rows) {
        try (Workbook workbook = new XSSFWorkbook();
             ByteArrayOutputStream out = new ByteArrayOutputStream()) {

            Sheet sheet = workbook.createSheet("Registro IVA acquisti");
            ...
            for (VatPurchaseRegisterRowView row : rows) {
                ...
                excelRow.createCell(2).setCellValue(nvl(row.getSupplierName()));
            ...
            throw new IllegalStateException("Errore nella generazione del file XLSX del registro IVA acquisti.", ex);
        }
    }
